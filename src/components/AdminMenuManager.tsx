      ...
      <div className="space-y-8">
            ...
            <div key={cat.id} className="border border-slate-200 rounded-2xl p-5 md:p-6 bg-white shadow-sm hover:shadow-md transition-shadow relative group">
                ...
                {editingCatId === cat.id ? (
                  ...
                  <>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button 
                        onClick={() => {
                          if (window.confirm(`คุณแน่ใจหรือไม่ที่จะลบหมวดหมู่ "${cat.name}" พร้อมเมนูทั้งหมดข้างใน?`)) {
                    ...
                    {editingItemId === item.id ? (
                      ...
                      <>
                        ...
                        <div className="flex items-center gap-2 opacity-0 group-hover/item:opacity-100 transition-opacity">
                          ...
                          <button onClick={() => onDeleteMenuItem(item.id)} className="text-slate-400 hover:text-rose-600 hover:bg-rose-50 p-2 rounded-lg transition-colors" title="ลบเมนูนี้">
                            <Trash2 size={18} />
                          </button>
                        </div>
                      </>
              ...
              <div className="flex flex-col sm:flex-row gap-3 mt-4 pt-4 border-t border-slate-100">
                <input 
                  ...
                  onKeyDown={(e) => {
                    if (e.key === 'Enter') {
                      onAddMenuItem(cat.id, newItemNames[cat.id]);
        ...
        <div className="border-2 border-dashed border-slate-300 rounded-2xl p-6 flex flex-col sm:flex-row gap-5 items-start sm:items-center bg-slate-50/50 hover:bg-slate-50 transition-colors mt-8">
          ...
          <div className="flex items-center justify-between w-full sm:w-auto gap-6">
            ...
            <button 
              onClick={() => { onAddCategory(newCatName, newCatReq); setNewCatName(''); }}
